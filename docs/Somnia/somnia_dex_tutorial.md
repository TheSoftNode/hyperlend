# Build a DEX on Somnia - Complete Tutorial
...
This guide is not an introduction to Solidity Programming; you are expected to understand:
...
**Trading Process:**
1. If a user wants to buy Token A with Token B:
2. They send Token B into the Pool
3. The Smart Contract calculates how much Token A to send out to maintain `x * y = k`
4. As more Token A is withdrawn, its price increases (slippage)

**Liquidity Provision:**
- Anyone can deposit an equal value of both tokens into the pool to become a Liquidity Provider (LP)
- LPs earn a share of the trading fees (e.g., 0.3%)
- AMMs are fully decentralized with no need for counterparties
- Open and permissionless to use and contribute liquidity
...
library SomniaLibrary {
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = ISomniaPair(pairFor(factory, tokenA, tokenB)).getReserves();
...
contract SomniaRouter {
    using SomniaLibrary for address;

    address public immutable factory;
    address public immutable WSTT; // Wrapped STT
    ...
    constructor(address _factory, address _WSTT) {
        factory = _factory;
        WSTT = _WSTT;
    }

    receive() external payable {
        assert(msg.sender == WSTT); // only accept STT via fallback from the WSTT contract
    }
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = SomniaLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = SomniaLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = SomniaLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquiditySTT(
        ...
        address to,
        uint deadline
    ) external virtual payable ensure(deadline) returns (uint amountToken, uint amountSTT, uint liquidity) {
        (amountToken, amountSTT) = _addLiquidity(
            token,
            WSTT,
            amountTokenDesired,
            msg.value,
        ...
        address pair = SomniaLibrary.pairFor(factory, token, WSTT);
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWSTT(WSTT).deposit{value: amountSTT}();
        assert(IWSTT(WSTT).transfer(pair, amountSTT));
        liquidity = ISomniaPair(pair).mint(to);
        // refund dust STT, if any
        if (msg.value > amountSTT) TransferHelper.safeTransferSTT(msg.sender, msg.value - amountSTT);
    }
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = ISomniaPair(pair).burn(to);
    ...
    function removeLiquiditySTT(
        ...
        address to,
        uint deadline
    ) public virtual ensure(deadline) returns (uint amountToken, uint amountSTT) {
        (amountToken, amountSTT) = removeLiquidity(
            token,
            WSTT,
            liquidity,
            amountTokenMin,
        ...
        IWSTT(WSTT).withdraw(amountSTT);
        TransferHelper.safeTransferSTT(to, amountSTT);
    }
