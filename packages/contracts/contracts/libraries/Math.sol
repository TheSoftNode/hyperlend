...
library Math {
     ...
     */
    function mulWad(uint256 a, uint256 b) internal pure returns (uint256 result) {
     ...
     */
    function divWad(uint256 a, uint256 b) internal pure returns (uint256 result) {
     ...
     */
    function percentageChange(uint256 oldValue, uint256 newValue) internal pure returns (int256 result) {
     ...
     */
    function clamp(uint256 value, uint256 lower, uint256 upper) internal pure returns (uint256 result) {
     ...
     */
    function approxEqual(uint256 a, uint256 b, uint256 tolerance) internal pure returns (bool result) {
        if (a == b) return true;
     ...
     */
    function lerp(uint256 a, uint256 b, uint256 t) internal pure returns (uint256 result) {
     ...
     */
    function standardDeviation(uint256[] memory values) internal pure returns (uint256 result) {
     ...
     */
    function sharpeRatio(uint256[] memory returns, uint256 riskFreeRate) internal pure returns (uint256 result) {
        if (returns.length == 0) revert Math__InvalidInput();
     ...
     */
    function correlation(uint256[] memory x, uint256[] memory y) internal pure returns (int256 result) {
    ...
    function calculateVaR(
        ...
        if (returns.length == 0 || confidenceLevel >= 100) revert Math__InvalidInput();
        
        // Sort returns (simple bubble sort for small arrays)
        for (uint256 i = 0; i < returns.length - 1; i++) {
            for (uint256 j = 0; j < returns.length - i - 1; j++) {
                if (returns[j] > returns[j + 1]) {
                    int256 temp = returns[j];
        ...
        // Find percentile
        uint256 index = ((100 - confidenceLevel) * returns.length) / 100;
        if (index >= returns.length) index = returns.length - 1;
        
        // Return absolute value of the loss
        result = returns[index] < 0 ? uint256(-returns[index]) : 0;
    }
     ...
     * @param riskFreeRate Risk-free rate (scaled)
     ...
     */
    function decimalToBps(uint256 decimal) internal pure returns (uint256 result) {
     ...
     */
    function annualizeRate(uint256 periodRate, uint256 periodsPerYear) internal pure returns (uint256 result) {
     ...
     */
    function deannualizeRate(uint256 annualRate, uint256 periodsPerYear) internal pure returns (uint256 result) {
        if (periodsPerYear == 0) revert Math__DivisionByZero();
